...
class SlideItemList(ItemList):
    ...
    def __init__(self, items:Iterator, path:PathOrStr='.', label_cls:Callable=None, inner_df:Any=None,
        ...
        # not needed any more for newer fast.ai versions: self.__post_init__()
...
def slide_regression_result(learn: Learner, image_count: int=5):
    with torch.no_grad():
        img_batch, target_batch = learn.data.one_batch(DatasetType.Train, False, False, False)
        prediction_batch = learn.model(img_batch)
        reg_batch = prediction_batch

        bbox_gt_batch, class_gt_batch = target_batch
        print('BS: ', img_batch.shape, reg_batch.shape, class_gt_batch.shape, bbox_gt_batch.shape)
        for img, bbox_gt, class_gt, reg  in \
                list(zip(img_batch, bbox_gt_batch, class_gt_batch, reg_batch))[:image_count]:
            img = Image(learn.data.denorm(img))
            ...
            pred_score_classes = '%.2f' % to_np(reg[0])
            print('Result: ' ,pred_score_classes)
            print('Reg:', reg)

            show_results_reg(img, bbox_gt, class_gt, (15, 3), title=str(pred_score_classes))
                
                

def show_results_reg(img, bbox_gt, preds_gt, figsize=(5,5)
                 , title: str=""):
    ...
    for bbox, c in zip(bbox_gt, preds_gt):
        draw_rect(ax, [bbox[1],bbox[0],bbox[3],bbox[2]], text=f'Mit')
    plt.title(title)
